...
local function start_git(args)
  ...
  git.watch_branch(function(branches)
    require("resurrect/ui").confirmation({
      prompt = ("Git branch changed %s -> %s, switch? [y/N]"):format(branches.current, branches.new),
      callback = function(yes)
        if yes then
          if M.active_session then
            stop()
          end
          require("resurrect/util").close_files() -- FIXME: this leaves an empty buffer sometimes
          start({ branches.new })
        end
      end,
    })
...
local function delete_session(arg)
  if #arg > 0 then
    ...
    if M.active_session == require("resurrect/util").session_shortname(s.name) then
      stop()
    end
...
function M.setup(opts)
  ...
  -- TODO: should this be loaded on a different event
  ...
  if M.config.add_command then
    require("resurrect/util").user_command("Resurrect", {
      default = resurrect,
